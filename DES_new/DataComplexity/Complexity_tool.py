...
def create_interpolation_data(group_data,group_label):
    ...
    for label in ulabel:
        ...
        if(len(label_data) > 10): # if samples number more than 10, random select a half samples
            ...
            selected_data = [label_data[inx] for inx in random_sequence[0: int(len(random_sequence) / 2) ]]
        else:
            selected_data = label_data
        grid_x, grid_y = np.mgrid[0:1:100j, 0:1:200j]

        # grid_z0 = griddata(points, values, (grid_x, grid_y), method='nearest')
        interpolated_data = np.vstack((interpolated_data, ovo_interpolation(selected_data)))
