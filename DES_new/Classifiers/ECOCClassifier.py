...
class SimpleECOCClassifier:
    ...
    """
    def __init__(self, estimator, code_matrix, decoder='HD', soft=True):
    ...
    def predict(self, X):
        ...
        ###############7-16 将此处_predict_binary改为_predict_binary_
        Y = np.array([_predict_binary_(self.estimators_[i], X) for i in range(len(self.estimators_))]).T
    ...
    def predict_(self, X, col_i):
        check_is_fitted(self, 'estimators_')
        pred = np.array([_predict_label(self.estimators_[i], X) for i in range(len(self.estimators_))])[col_i]
