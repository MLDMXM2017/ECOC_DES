...
class DCN2(DataComplexity):
	...
	def _score(self, X, y):
		...
		if sc_count == 0:
			sc_count = 1
		if dc_distance == 0:			#这里挺迷的，我觉得应该是直接返回无穷大就好......
			dm = distance.copy()
			dm[dm == np.inf] = 0
			dc_distance = dm.mean()
			print(a)

		return sc_distance / dc_distance
		#return 1 / ((sc_distance / sc_count) / (dc_distance / dc_count))	这里也是反过来了(明明是越小越好)
...
class DCN3(DataComplexity):
	...
	def _score(self, X, y):
		...
		return error_rate
		#return 1 - error_rate	#这里应该是直接用错误率来表示啊
